...
namespace Gibbed.Dunia.FileFormats
{
    public class LegacyBigFile
    {
        ...
        public Endian Endian
        {
            get
            {
                return (this.Platform == LegacyBig.Platform.PC || this.Platform == LegacyBig.Platform.Any)
                           ? Endian.Little
                           : Endian.Big;
            }
        }
        ...
        public int Version;
        public LegacyBig.Platform Platform;
        // ReSharper disable InconsistentNaming
        public uint Unknown74;
        // ReSharper restore InconsistentNaming
        public readonly List<LegacyBig.Entry> Entries = new List<LegacyBig.Entry>();
        public readonly List<LegacyBig.SubFatEntry> SubFats = new List<LegacyBig.SubFatEntry>();
        ...
        public void Serialize(Stream output)
        {
            ...
            if (this.Platform != LegacyBig.Platform.Any &&
                this.Platform != LegacyBig.Platform.PC &&
                this.Platform != LegacyBig.Platform.X360 &&
                this.Platform != LegacyBig.Platform.PS3)
            {
        ...
        public void Deserialize(Stream input)
        {
            ...
            var platform = LegacyBig.Platform.Invalid;
            uint unknown74 = 0;

            if (version >= 3)
            {
                unknown74 = input.ReadValueU32(Endian.Little); // ###### ##
                platform = (LegacyBig.Platform)(unknown74 & 0xFF);
                unknown74 >>= 8;
            }

            if (platform != LegacyBig.Platform.Any &&
                platform != LegacyBig.Platform.PC &&
                platform != LegacyBig.Platform.X360 &&
                platform != LegacyBig.Platform.PS3)
            {
                throw new FormatException("unsupported/invalid platform");
            }
            ...
            {
                var subFat = new LegacyBig.SubFatEntry();
        ...
        internal static void SanityCheckEntry(LegacyBig.Entry entry, LegacyBig.Platform platform)
        {
            if (entry.CompressionScheme == LegacyBig.CompressionScheme.None)
            {
                if (platform != LegacyBig.Platform.X360 &&
                    entry.UncompressedSize != 0)
                {
                    throw new FormatException("got entry with no compression with a non-zero uncompressed size");
                }
            }
            else if (entry.CompressionScheme == LegacyBig.CompressionScheme.LZO1x ||
                     entry.CompressionScheme == LegacyBig.CompressionScheme.Zlib)
            {
        ...
        private static bool IsValidPlatform(LegacyBig.Platform platform, uint unknown74)
        {
            if (platform == LegacyBig.Platform.Any &&
                unknown74 != 0)
            {
            ...
            if (platform == LegacyBig.Platform.PC &&
            ...
            if (platform == LegacyBig.Platform.PS3 &&
                (unknown74 < 1 || unknown74 > 4))
            {
                return false;
            }
        ...
        internal LegacyBig.IEntrySerializer EntrySerializer
        {
            get
            {
        ...
        private static readonly ReadOnlyCollection<LegacyBig.IEntrySerializer> _EntrySerializers;

        static LegacyBigFile()
        {
            _EntrySerializers = new ReadOnlyCollection<LegacyBig.IEntrySerializer>(new LegacyBig.IEntrySerializer[]
            {
                null, // 0
                new LegacyBig.EntrySerializerV1(), // 1
                new LegacyBig.EntrySerializerV1(), // 2
                new LegacyBig.EntrySerializerV1(), // 3
                new LegacyBig.EntrySerializerV1(), // 4
                new LegacyBig.EntrySerializerV5(), // 5
                new LegacyBig.EntrySerializerV6V8(), // 6
                new LegacyBig.EntrySerializerV7(), // 7
                new LegacyBig.EntrySerializerV6V8(), // 8
                new LegacyBig.EntrySerializerV9(), // 9
            });
        }
    }
}
