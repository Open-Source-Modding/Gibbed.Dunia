...
namespace Gibbed.Dunia.FileFormats.Big
{
    ...
    // Index may be XTEA-encrypted (the 0x80000000 flag bit).
    internal class BigFileLayoutV11<T> : IArchiveLayout<T>
    {
        ...
        public Stream ReadIndex(Stream input, Endian endian, bool indexIsEncrypted, int entryCount, IEntrySerializer<T> entrySerializer)
        {
            ...
            indexSize &= ~7;
            var indexKey = Crypto.GenerateXTEAKey((uint)indexSize);
            Gibbed.Dunia.FileFormats.Crypto.XTEA.Decrypt(indexBytes, 0, indexSize, indexKey);
            return new MemoryStream(indexBytes, false);
        }
